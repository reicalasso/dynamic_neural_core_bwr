"""
Simple training script for the basic DNC model.

This script demonstrates how to train the basic DNC model on a simple task.
"""
...
class SimpleCopyDataset(Dataset):
    ...
    def __init__(self, num_samples=1000, seq_len=32, vocab_size=100):
...
class BasicDNCTrainer:
    """Trainer for the basic DNC model."""
...
def create_config():
    ...
    return {
        'model': {
            'vocab_size': 128,  # 126 tokens + 1 start token + 1 end token
            'd_model': 128,
            'n_layers': 2,
            'n_heads': 4,
            'slots': 64,
...
def main():
    ...
    trainer = BasicDNCTrainer(config)
