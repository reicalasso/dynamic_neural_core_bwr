"""
Basic DNC model implementation.
"""
...
class BasicDNC(BaseModel):
    ...
    def __init__(self, vocab_size, d_model=256, n_layers=4, n_heads=4, slots=128, max_seq_len=512):
        ...
        # Memory bank
        self.memory_manager = MemoryManager("simple", d_model=d_model, slots=slots)
    ...
    def forward(self, input_ids, return_memory_info=False):
        ...
        memory_read, attention_weights = self.memory_manager.read(memory_query)  # [B, T, D]
        ...
        if self.training:
            self.memory_manager.write(x.detach(), attention_weights)
